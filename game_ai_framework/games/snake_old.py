...
Following the BaseGame interface for the AI framework
"""
...
class SnakeGame(BaseGame):
    ...
    def __init__(self, grid_size: int = 20, cell_size: int = 20, render: bool = True):
        ...
        Args:
            ...
            render: Whether to create display
        """
    ...
    def step(self, action: int) -> Tuple[GameState, float, bool, Dict[str, Any]]:
        ...
        """
        if self.game_over:
            return self._get_game_state(), 0, True, {"score": self.score}
        ...
        if (new_head[0] < 0 or new_head[0] >= self.grid_size or
            ...
            self.game_over = True
            return self._get_game_state(), -10, True, {"score": self.score}
        ...
        return self._get_game_state(), reward, self.game_over, info
    ...
    def render(self) -> None:
        ...
        for x in range(0, self.screen_size, self.cell_size):
            pygame.draw.line(self.screen, (40, 40, 40), (x, 0), (x, self.screen_size), 1)
        for y in range(0, self.screen_size, self.cell_size):
            pygame.draw.line(self.screen, (40, 40, 40), (0, y), (self.screen_size, y), 1)
        ...
        if hasattr(self, 'clock'):
            self.clock.tick(10)  # 10 FPS for Snake
    
    def get_state_vector(self) -> np.ndarray:
        """Get state as vector for neural network
        
        State includes:
        - Direction of food relative to head (2)
        ...
        wall_distances = [
            head[1] / self.grid_size,  # Up
            (self.grid_size - 1 - head[0]) / self.grid_size,  # Right
            (self.grid_size - 1 - head[1]) / self.grid_size,  # Down
            head[0] / self.grid_size  # Left
        ]
        state.extend(wall_distances)
        
        # Body distances in 4 directions
        body_distances = []
        for dx, dy in [(0, -1), (1, 0), (0, 1), (-1, 0)]:
            dist = 1.0
            for i in range(1, self.grid_size):
                check_pos = (head[0] + i*dx, head[1] + i*dy)
                if check_pos in self.snake[1:]:  # Exclude head
    ...
    def analyze_task_context(self) -> TaskContext:
        ...
        # Avoidance priority - high when surrounded by dangers
        avoidance_priority = min(danger_count / 2, 1.0)
        
        # Navigation priority - high when far from food
        food_dist = abs(head[0] - self.food[0]) + abs(head[1] - self.food[1])
        navigation_priority = min(food_dist / (self.grid_size * 2), 1.0)
        
        # Combat priority - not applicable in Snake
        combat_priority = 0.0
        
        # Survival priority - increases with snake length (harder to navigate)
        survival_priority = len(self.snake) / (self.grid_size * self.grid_size)
        
        # Collection priority - food collection
    ...
    def get_semantic_features(self) -> Dict[str, float]:
        ...
        # Threat level - based on nearby obstacles
        threats = 0
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if dx == 0 and dy == 0:
    ...
    def close(self) -> None:
        """Clean up resources"""
        if self.render_mode and hasattr(self, 'screen'):
            pygame.quit()
    ...
    def _get_game_state(self) -> GameState:
        ...
        return GameState(
            raw_state=self.get_state_vector(),
...
if __name__ == "__main__":
    ...
    print(f"State vector shape: {state.raw_state.shape}")
